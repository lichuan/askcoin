...
bool Wsock_Node::start(std::string host, uint16 port)
{
    ...
    std::unique_ptr<fly::net::Server<Wsock>> server(new fly::net::Server<Wsock>(fly::net::Addr(host, port),
                                                                                ...
                                                                                cpu_num, 1840)); // todo size in the future ?
    if(server->start())
    {
...
void Blockchain::do_wsock_message(std::unique_ptr<fly::net::Message<Wsock>> &message)
{
    ...
    if(type == net::api::MSG_ACCOUNT)
    {
        ...
        else if(cmd == net::api::ACCOUNT_PROBE)
        {
            ...
            if(!user->m_reg_probe)
            {
                ...
                // because this tx doesn't save to leveldb,
                // so don't use verify_tx_sign here
    ...
    if(type == net::api::MSG_EXPLORER)
    {
        ...
        else if(cmd == net::api::EXPLORER_QUERY)
        {
            ...
            uint32 query_type = doc["type"].GetUint();
            ...
            else if(query_type == 2 || query_type == 3)
            {
                ...
                rsp_doc.AddMember("type", query_type, allocator);
            ...
            else if(query_type == 4)
            {
                ...
                fly::base::Scope_CB scb(
                    [&] {
                        ...
                        rsp_doc.AddMember("msg_cmd", net::api::EXPLORER_QUERY, allocator);
                        rsp_doc.AddMember("msg_id", msg_id, allocator);
                        rsp_doc.AddMember("type", 4, allocator);
                        connection->send(rsp_doc);
                    },
                    [] {});
                char raw_hash[33] = {0};
                uint32 len = fly::base::base64_decode(tx_hash.c_str(), tx_hash.length(), raw_hash, 33);
                ...
                if(m_merge_point->m_import_block_id > 0)
                {
                    if(block_id + 100 <= m_merge_point->m_import_block_id)
                    {
                        ASKCOIN_RETURN;
                    }
                }
                
                const uint64 DISTANCE = 100;
                bool left_end = false;
            ...
            if(doc.HasMember("block_id_from"))
            {
                ...
                for(uint64 id = block_id_from; id <= block_id_to; ++id)
                {
                    ...
                    for(uint32 i = 0; i < tx_num; ++i)
                    {
                        ...
                        if(!verify_tx_sign(pubkey, tx_id, tx_sign, block_id))
                        {
                            CONSOLE_LOG_FATAL("EXCHANGE_LOGIN verify tx sign from leveldb failed, tx_id: %s", tx_id.c_str());
                            ASKCOIN_EXIT(EXIT_FAILURE);
                        }
    ...
    if(tx_type == 1)
    {
        ...
        std::string tx_id = tx_hash_b64(buffer.GetString(), buffer.GetSize(), block_id);
